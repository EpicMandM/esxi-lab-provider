...
Prebuilt dev container image (linux/amd64): `ghcr.io/epicmandm/esxi-lab-provider/devcontainer:latest` (built on push to `main` when `.devcontainer/` changes, on a biweekly schedule if there were changes in the last 14 days, or via **Actions → Devcontainer → Run workflow**).
